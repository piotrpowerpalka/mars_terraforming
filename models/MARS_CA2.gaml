...
global {
    ...
    float b0 -> 0.012881263446187147;
	float b1 -> -5.07154313e-04;
	float b2 -> -1.28182694e-03; 
	float b3 -> 4.98042378e-06;
	float b4 -> -7.68025808e-06; 
	float b5 -> 5.41798872e-06;
...
species cell {
	...
	action move_gas_regression
	{
		...
		loop i from: 0 to: length(neighbours)-1
		{
			float xa <- min( abs( azim[i] - dw),  abs( 360 + ( azim[i] - dw ) ) );
			
			float delta_co2_column <- b0 + b1*xa + b2*xs + b3*xa^2 + b4*xa*xs + b5*xs^2;
