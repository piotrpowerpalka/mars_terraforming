...
species cell {
	...
	aspect base{
		//draw sphere(0.01) at: point(x,y,z) color: rgb(int(temp), int(temp),int(temp)) border: #black;
		//draw sh at: point(100*x,100*y,100*z) color: rgb(int(temp),int(temp),int(temp)) border: #black;
		draw sh color: #red border: #black;
	}
	
	reflex initial when: cycle = 0 {
		neighbours <- cell where (each.id_cell in neigh);
		if (spec != 1) {
    		sh <- polygon([{(neighbours[0].x + longitude)/2, (neighbours[0].latitude + latitude)/2}, 
	...
	reflex greenhouse when: cycle > 0 and cycle mod 2 * 668 = 0 { // obliczenia co 1 rok marsjanski
