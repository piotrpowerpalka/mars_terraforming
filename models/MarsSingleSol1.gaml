...
species cell parallel: true {
	...
	reflex _balance when: cycle > 1 and cycle mod 2 = 0 {
		...
		//Ts <- prevTs + 1/(1292.0 * (pCO2 - frozenCO2) * delta_h2) 
		Ts <- prevTs + 1/(1292.0 * (pCO2 - frozenCO2) ) // bez mnożenia przez powierzchnię - jest to flux  
...
experiment main_experiment until: (cycle <= 100) 
{
	...
	output {
		...
		display chart1 refresh: every(2#cycles) {
			chart "Frozen CO2" type: series background: #lightgray style: exploded {
				data "Frozen CO2" value: tempFrozen color: #blue;  
			}
		}
		display chart2 refresh: every(2#cycles) {
			chart "Temperature" type: series background: #lightgray style: exploded {
