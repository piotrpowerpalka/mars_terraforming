...
global {
	...
	int sollon_number <- 0;
    int model_number <- 3;
    int selected_cell <- 1513;
    int selected_sol <- 510;
	...
	float CO2freezingCoeff <- 5e-6 / step_sol; // percentage part of CO2 being sublimated / resublimated per sol
    		...
    		int sol <-  (cycle -1) / (2 * step_sol);
 	 			write "" + ( (cycle -1) / (2 * step_sol) ) + ";" + n.id_cell+ ";" + 
	 						n.Ts + ";" + n.pCO2 + ";"  +
    	          	 		...
    	          	 		n.insol + ";" + n.Tps + ";"  +
    	          	 		n.kat + ";" + n.latitude +  ";" + n.rad_en + ";" + n.green_en + ";" + n.eddy_Ts  + ";" + n.hadley_Ts + ";" + n.latentCO2;
...
//										  Ks-1 
	...
	reflex diurnal_real when: false and cycle mod 2 = 0 and selected_cell = id_cell and (selected_sol = sol_year or (selected_sol + 1) = sol_year) and year = 1 {
		float fi <- latitude;
		if (fi = 90.0)  { fi <- 89.99; } 
		if (fi = -90.0) { fi <- -89.99; }
		...
		//write "" + id_cell + ";" + sol_year + ";" + sol_lon + ";" + kat + ";" + fi + ";" + Ts + ";" + pCO2 + ";"  
		//		 + rad_en + ";" + green_en + ";" + eddy_Ts  + ";" + hadley_Ts + ";" + latentCO2;
		
		float Tsol <- Ts;
		int dt <- 48; // pół sola
		if (frozenCO2 > 0.0) {
			write "CO2 ice detected - trees are dead :(";
			return;
		}
		float _energy <- 0;
		
		loop t from: 0 to: dt {
			float insol <- (1.0 ) *(1.0/#pi) * S0  * ((1 + e * cos((sol_lon - 248) mod 360))^2) / ((1 - e^2)^2) 
			* max(0.0, sin(fi) * sin(nachylenieOsi) * sin(sol_lon) 
				* 15.0 * 2.0 * #pi / 360.0 + cos(fi) * cos(kat) 
				* ( sin(7.5 * (t + 1.0)) - sin(7.5 * t) )
		     ) ;
			...
			Tsol <- Tsol + (insol_en + (-rad_en + green_en + eddy_Ts + hadley_Ts) / dt) * (delta_t /dt) / (cCO2       * pCO2 + cSoil     * soilDepth * soilDensity  / 2);
			//write "" + insol + ";" + Tsol;
		}
	...
	reflex diurnal_simple when: false and cycle mod 2 = 0 and selected_cell = id_cell and selected_sol = sol_year and year = 1 {
		float Tsol <- Ts;
		int dt <- 24; // pół sola
		if (frozenCO2 > 0.0) {
			write "CO2 ice detected - trees are dead :(";
		...
		loop times: dt {
			Tsol <- Tsol + (-rad_en + green_en + eddy_Ts + hadley_Ts) * (delta_t / (2*dt)) / (cCO2       * pCO2 + cSoil     * soilDepth * soilDensity);
			write Tsol;
		}
...
experiment main_experiment until: (cycle > 6680) 
{
	...
	parameter "Selected sol" var: selected_sol;
