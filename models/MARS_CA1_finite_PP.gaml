...
global {
	...
	init {
		create cell from: csv_file("../includes/out_grid_4002_0h_" + sollon_number + "_sollon_2.csv", ";", true) with:
    	[   id_cell::int(read("id")),
    		x::int(read("x")),
    		y::int(read("y")),
    		z::int(read("z")),     	
    ...
    reflex saving_ when: cycle > 0 and cycle mod 2 = 0 {    	
    	loop n over: cell {
    	    ...
    	    ] to: "../results/finite_fixed/sol" + sollon_number + "/save_csv_" + (cycle / 2) + ".csv" rewrite: false type: "csv";
   	 	}	
   	 }
}
...
species cell {
	...
	float height_diff <- 0.0;
		...
		next_step_co2_column <- next_step_co2_column - (height_diff * 0.00945 + 0.05275);
