...
global {
    ...
    reflex when: cycle > 1 and cycle mod 2 = 1 and cycle <= 1336 {
		...
		co2_regress_fct  <- build(instances);
		co2_fct_mat[solno] <- co2_regress_fct;
...
species cell {
	...
	reflex finite_element when: cycle > 0 and cycle mod 2 = 1 and model_number = 3
	{
		...
		if( alfa = beta ){
			...
			else if (eddy_calculated = 1){
				float eddy_co2 <- A_eddy * Ts^1.5 / (atm_press / 101325.0 * 3.3) *  sqrt(1.0/44.0 + 1.0/44.0) * delta_t / delta_h2;
				float eddy_nh3 <- A_eddy * Ts^1.5 / (atm_press / 101325.0 * 0.5 * (3.3 + 2.6) ) *  sqrt(1.0/44.0 + 1.0/17.0) * delta_t / delta_h2;
				float eddy_ch4 <- A_eddy * Ts^1.5 / (atm_press / 101325.0 * 0.5 * (3.3 + 3.8) ) *  sqrt(1.0/44.0 + 1.0/16.0) * delta_t / delta_h2;
				float eddy_cfc <-  A_eddy * Ts^1.5 / (atm_press /101325.0 * 0.5 * (3.3 + 4.5) ) *  sqrt(1.0/44.0 + 1.0/120.0) * delta_t / delta_h2;
				
				next_step_co2_column <- -(beta_dw / delta_h)*(neighbours[beta_az].co2_column - co2_column) - (alfa_dw / delta_h)*(neighbours[alfa_az].co2_column - co2_column) + eddy_co2 * (param*neigh_co2_sum - 4*co2_column) + co2_column;
				nextstep_cfc_column <- -(beta_dw / delta_h)*(neighbours[beta_az].cfc_column - cfc_column) - (alfa_dw / delta_h)*(neighbours[alfa_az].cfc_column - cfc_column)  + eddy_cfc * (param*neigh_cfc_sum - 4*cfc_column) + cfc_column;
				nextstep_ch4_column <- -(beta_dw / delta_h)*(neighbours[beta_az].ch4_column - ch4_column) - (alfa_dw / delta_h)*(neighbours[alfa_az].ch4_column - ch4_column)  + eddy_ch4 * (param*neigh_ch4_sum - 4*ch4_column) + ch4_column;
				nextstep_nh3_column <- -(beta_dw / delta_h)*(neighbours[beta_az].nh3_column - nh3_column) - (alfa_dw / delta_h)*(neighbours[alfa_az].nh3_column - nh3_column)  + eddy_nh3 * (param*neigh_nh3_sum - 4*nh3_column) + nh3_column;				
			}
