...
global {
	...
	float K_ <- 0.0 * delta_t / delta_h2 / step_sol; // K * delta_t / delta_h2
	float Wind <- 0.0;
     ...
     */
    reflex saving_ when: cycle > 1 and false  and (
 ...
 //   	    if    ((cycle - 1) / 2 >= 591 and (cycle - 1) / 2 <= (591 + 20) // mid N winter  , za: https://mars.nasa.gov/all-about-mars/facts/mars-year/
//    	    	or (cycle - 1) / 2 = 283 // mid N summer
 //   	    ) 
 			//{
...
species cell parallel: true {
	...
	reflex finite_element when: cycle > 0 and cycle mod 2 = 1
	{
		...
		if( alfa = beta ){
			...
			float alfa_dw <- Wind * zonal_wind * (sin(beta) - cos(beta)) / sin(beta - alfa);
			float beta_dw <- Wind * merid_wind * (cos(alfa) - sin(alfa)) / sin(beta - alfa);
		
			float neigh_Ts_sum <- 0;
			
			loop n over: neighbours {
				neigh_Ts_sum <- neigh_Ts_sum + n.Ts;
			}
			eddy_Ts <-  -(beta_dw / delta_h)*(neighbours[beta_az].Ts - Ts) - (alfa_dw / delta_h)*(neighbours[alfa_az].Ts - Ts) + K_* (param*neigh_Ts_sum - 4*Ts);
		}
	}
	...
	reflex _balance when: cycle > 1 and cycle mod 2 = 0 {
		...
		Ts <- prevTs + div_Ts + eddy_Ts + 1/(1292.0 * (pCO2 - frozenCO2) ) // without multiplication by area - its flux  
							* ( (1 - 0.2) * insol - (0.9 * sigma) * prevTs^4 - sublimateCO2 + resublimateCO2);
			
		energy <- Ts * ( 1292.0 ) ; // + soil_thickness * c_s);
	 
		//if (Ts < 140.0) { Ts <- 140.0; } // twarde ograniczenie na temp
...
experiment main_experiment until: (cycle <= 100) 
{
	...
	output {
		...
		display mars_plain_fCO2 type: opengl ambient_light: 100 background: #white  
		{
			species cell aspect: plain_FrozenCo2;
		}	
		
	
		display chart1 refresh: every(2#cycles) {
		...
		display chart2 refresh: every(2#cycles) {
			chart "Temperature" type: series background: #white style: exploded {
				...
				data "Eddy temperature" value: eddyTemp color: #purple;  
			}
		}
		...
		display chart4 refresh: every(2#cycles) {
			chart "Energy" type: series background: #white style: exploded {
				data "Energy" value: tempEnergy color: #green;  
			}
		}
		display chart5 refresh: every(2#cycles) {
			chart "Total CO2" type: series background: #white style: exploded {
				data "Total CO2" value: cell sum_of(each.pCO2 + each.frozenCO2)/numOfHexes color: #green;  
			}
		}
		/*
		display mars_frozenCO2 type: opengl camera_interaction: true ambient_light: 100 
			background: #black orthographic_projection: true rotate: 15.0 
		{
			species cell aspect: sphere_frozenCo2;
		} */
		...
		monitor "Sum of CO2" name: sumCO2 value: cell sum_of(each.pCO2 + each.frozenCO2);
