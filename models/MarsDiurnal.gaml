...
global {
	...
	int mode <- 0;  // 0: daily mode, 1: hourly mode
	...
	float CO2freezingCoeff <- 0.000005; // percentage part of CO2 being sublimated / resublimated per sol
    ...
    reflex change_resolution when: false and year = 1 and sol_year = 0 {
    	step_sol <- 24;
    	step <- 88775#sec / step_sol;
    	mode <- 1;
    }
     ...
     */
    reflex saving_output when: log_output and year >= log_first_year and hour = 0 and (
    ...
    reflex hadley_heat_transport when: hour = 0 and cycle > 0 and cycle mod 2 = 1 {
    	hadleyN <- 0.0;
    	hadleyS <- 0.0;
    	loop n over: cell {
    	...
    	loop n over: cell {
    		if (n.hadley_ht > 0.0 and n.latitude > 0) {
    			ask n {
    				hadley_Ts <- hadley_ht * Ts * hadleyParam / step_sol;
    			}
...
species cell parallel: true {
	...
	list<float> py_fco2 <- list_with(668, 0.0);
	...
	aspect plain_insol{
		draw circle(1.0) at: {(-longitude+180.0)/3.6, (-latitude+90.0)/1.8} color: insol<=0.0?#black:#yellow border: #black;	
	}
	...
	reflex finite_element when: hour = 0 and  cycle > 0 and cycle mod 2 = 1
	{
	...
	reflex day when: cycle > 1 and hour = 0 {
	...
	reflex _balance_hourly when: false and cycle > 1 and cycle mod 2 = 0 and mode = 1 {
		
		heat_flux <- energy;
		
		insol <- (1.0 ) *(1.0/#pi) * S0  * ((1 + e * cos((sol_lon - 248) mod 360))^2) / ((1 - e^2)^2) 
			* max(0.0, sin(fi) * sin(nachylenieOsi) * sin(sol_lon) 
				* 15.0 * 2.0 * #pi / 360.0 + cos(fi) * cos(kat) 
				* ( sin(longitude + 15.0 * (hour + 24.0 / step_sol)) - sin(longitude + 15.0 * hour) )
		     ) ; 		
		...
		prev_en  <- prevTs * (cCO2       * pCO2 + cSoil     * soilDepth * soilDensity) / ( delta_t / step_sol ); 
		
		// zamiast obliczen - przypisanie
		frozenCO2 <- py_fco2[sol_year];
		latentCO2 <- py_fco2[(sol_year) mod 668] - py_fco2[(sol_year + 667) mod 668];
	//	if (id_cell = 2859) {
	//		write "fco2[" + id_cell + "]["+sol_year+"] = " + py_fco2[sol_year] + " " + frozenCO2;		
	//	}
		
		energy <- insol_en - rad_en + green_en + eddy_Ts + hadley_Ts  + latentCO2;  // latent heat
						   		
		Ts <- prevTs			// previous step temp. 
				+  energy * ( delta_t / step_sol) / (cCO2       * pCO2 + cSoil     * soilDepth * soilDensity); // mass m-2 * conduction - without multiplication by area - its flux
...
//										  Ks-1 
	...
	reflex _bh when: false and cycle > 1 and cycle mod 2 = 0 and mode = 1 {
		...
		if (id_cell != 1513){
			insol <- (1.0/#pi) * S0 * solar_const_modifier * ((1 + e * cos((sol_lon - 248) mod 360))^2) / ((1 - e^2)^2) 
				* max(0.0, sin(fi) * sin(nachylenieOsi) * sin(sol_lon) 
				* OMEGA * 2 / 24 * #pi / 360.0 + cos(fi) * cos(kat) * sin(OMEGA)
				) ; 		
		} else {
			insol <- (1.0 ) *(1.0/#pi) * S0  * ((1 + e * cos((sol_lon - 248) mod 360))^2) / ((1 - e^2)^2) 
			* max(0.0, sin(fi) * sin(nachylenieOsi) * sin(sol_lon) 
...
//										  Ks-1 
     ...
     */
	reflex _balance when: cycle > 1 and cycle mod 2 = 0 and hour = 0 and mode = 0 {
		heat_flux <- energy;
...
experiment main_experiment until: (cycle > 6680) 
{
	...
	parameter "Start sollon" var: sollon_number min: 0 max: 668;
...
//	parameter "Selected cells" var: selected_cells;
	parameter "Steps per sol" var: step_sol;
	...
	output {
		...
		{
			species cell aspect: plain_insol;
		}	
		...
		monitor "Hour"     name: h value: hour;
